...
def main():
    ...
    excluded_params = wrap_exclude_dict(
        excluded_dirs=args.exclude_dirs,
        excluded_files=args.exclude_files,
        excluded_extensions=args.exclude_extensions
    )

    doc_content, file_count = export_doc(args, excluded_params)

    dest_file_path = Path(args.output)
    dest_file_path.write_text(doc_content, encoding="utf-8")
...
def export_doc(args, excluded_params):
    ...
    for file_path in tqdm(filter_dirs(
        ...
        relative_path = Path(file_path).resolve().relative_to(base_dir)

        concatenated_text += current_template.format(
            relative_path,
            lang,
            content
        )
        file_count += 1
    concatenated_text += f"\n---\n\n__Total files processed:__ {file_count}\n"

    return concatenated_text, file_count
